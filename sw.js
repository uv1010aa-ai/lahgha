...
self.addEventListener("activate", event => { event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k.startsWith("lahgha-") && k !== "lahgha-v130").map(k => caches.delete(k))))); });
